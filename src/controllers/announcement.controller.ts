...
export class AnnouncementController {
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (!senderId) {
        res.status(400).json({
          success: false,
          error:
            "Sender ID is required. Provide it via x-sender-id header or senderId in body/query",
        });
        return;
      }
  ...
  getAnnouncementsByEvent = asyncHandler(
    async (req: Request, res: Response) => {
      const announcements = await announcementService.getAnnouncementsByEvent(
        req.params.eventId
      );
      res.json({
        success: true,
  ...
  getUserAnnouncements = asyncHandler(async (req: Request, res: Response) => {
